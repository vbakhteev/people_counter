...
def letterbox(img, height=608, width=1088, color=(127.5, 127.5, 127.5)):
    ...
    h, w = img.shape[:2]  # shape = [height, width]
    ratio = min(height / h, width / w)
    new_w = round(w * ratio)
    new_h = round(h * ratio)
    dw = (width - new_w) / 2  # width padding
    dh = (height - new_h) / 2  # height padding
    top, bottom = round(dh - 0.1), round(dh + 0.1)
    left, right = round(dw - 0.1), round(dw + 0.1)
    img = cv2.resize(img, (new_w, new_h), interpolation=cv2.INTER_AREA)  # resized, no border
